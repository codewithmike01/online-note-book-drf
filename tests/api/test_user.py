...
@pytest.mark.django_db
def test_user_get_current_user(user):
    ...
    assert cur_user_response["first_name"] == user.first_name
    assert cur_user_response["last_name"] == user.last_name
    assert cur_user_response["email"] == user.email
...
@pytest.mark.django_db
def test_user_verify_email(user):
    # Generate email verification token
    token = services.create_token(user.id)
...
@pytest.mark.django_db
def test_user_req_password_email_reset(user):
    # Generate email verification token
    token = services.create_token(user.id)
...
@pytest.mark.django_db
def test_user_confirm_password_email_reset(user):
    # Generate email verification token
    token = services.create_token(user.id)
    ...
    user_data = services.check_user_email(user.email)
    ...
    assert password_confirm_response["first_name"] == user.first_name
    assert password_confirm_response["last_name"] == user.last_name
    assert password_confirm_response["email"] == user.email
